...
canon() {
    # Full canonicalization: resolves symlinks + "." + ".." + redundant/
    # trailing slashes for every path shape rm accepts (absolute, relative,
    # "..", ".", trailing slash), without requiring the target to exist
    # (rm itself may target nonexistent paths, e.g. `rm -f missing`).
    realpath -m -- "$1"
}
