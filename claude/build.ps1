...
$enabledLanguages = Resolve-LanguageSelection -Enable $Enable -Disable $Disable -Supported @('go', 'dotnet', 'python', 'pwsh') -EnableSpecified ($PSBoundParameters.ContainsKey('Enable')) -DisableSpecified ($PSBoundParameters.ContainsKey('Disable'))
...
foreach ($language in @('go', 'dotnet', 'python', 'pwsh')) {
    $buildArgs += '--build-arg'
