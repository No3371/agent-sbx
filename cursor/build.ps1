# Build the custom Cursor CLI sandbox template with podman.
#
...
param(
    ...
    [string]$Image,            # e.g. cursor-custom:v1 (local) or docker.io/<user>/cursor-custom:v1 (push)

    [string]$Tar,              # optional path to export tar (e.g. .\cursor-custom.tar)
    [switch]$Push,
    ...
    [string]$HostCursorDir = '',   # passed through to prepare.ps1; default: $env:USERPROFILE\.cursor
    [string]$Destination   = ''    # passed through to prepare.ps1; default: <repo>/context/.cursor
)
...
# Input validation
if ($Image -match '<user>') {
    throw "Replace <user> in -Image with your registry username (e.g. docker.io/yourname/cursor-custom:v1)."
}
$loadTargets = @()
...
if (-not $SkipPrepare) {
    ...
    $prepareArgs = @()
    if ($HostCursorDir) { $prepareArgs += '-HostCursorDir'; $prepareArgs += $HostCursorDir }
