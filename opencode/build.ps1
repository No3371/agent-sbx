...
[CmdletBinding()]
param(
    [string]$Image = "sbx-opencode:v1",            # e.g. opencode-custom:v1

    [string]$Tar,              # optional path to export tar (e.g. .\opencode-custom.tar)
...
# Input validation
if ($Image -match '<user>') {
    throw "Replace <user> in -Image with your registry username (e.g. docker.io/yourname/opencode-custom:v1)."
}
$loadTargets = @()
...
$enabledLanguages = Resolve-LanguageSelection -Enable $Enable -Disable $Disable -Supported @('go','python','pwsh') -EnableSpecified ($PSBoundParameters.ContainsKey('Enable')) -DisableSpecified ($PSBoundParameters.ContainsKey('Disable'))
...
foreach ($language in @('go','python','pwsh')) {
    $buildArgs += '--build-arg'
